...
shinyServer( function(input, output) {
  ...
  output$distributionsPlot <- renderPlot({
    ...
    ## Function to find alpha optimizing
    # find.alpha <- function( alpha, mode, l.inf, l.sup, conf = 0.95 ){
    #   
    #   ## beta is function of alpha
    ...
    # alpha <- optimize( find.alpha, interval = seq(1, 1e3)
    #                    , mode = mode.pri, l.inf = inf, l.sup = sup )
    # 
    # ## May crush with some wide priors
    # if( alpha$objective > 1e-3){
    #   warning("Bad prior parameters estimation. Setting alpha = 1.")
    #   alpha <- 1
    # }else{
    #   alpha <- alpha$min
    # }
    ...
    fun.mean <- function( theta0, alpha, beta, n, k ){
      ##theta0 <- seq( from  = 0.01, to = 0.999, l = 1000)
